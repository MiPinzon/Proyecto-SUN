...
# Con estas funciones se crean tablas en la base de datos
try:
...
try:
    lector.execute("CREATE TABLE MATERIAS (CODIGO VARCHAR(9) PRIMARY KEY, NOMBRE VARCHAR(30), CODFACULTAD INTEGER(4), CODDEPA INTEGER(8), CREDITOS INTEGER(1), PREREQ VARCHAR(30), CODPREREQ VARCHAR(9))")
except:
    pass

try:
    lector.execute('''CREATE TABLE MATERIASDOC (CODIGO VARCHAR(9), NOMBRE VARCHAR(30), NOMDOC VARCHAR(56), 
    APDOC VARCHAR(24), ID VARCHAR(10), HORARIOINIT INTEGER(4), HORASCLASE INTEGER (2), DIAS VARCHAR(10), CUPOS INTEGER(3))''')
except:
    pass
...
try:
    ...
    NOMEST VARCHAR(56), APEST VARCHAR(24), IDDOC VARCHAR(10), 
    NOMDOC VARCHAR(56), APDOC VARCHAR(24), HORARIOINIT INTEGER(4), HORASCLASE INTEGER(2),
    DIAS VARCHAR(10), ESTATUS INTEGER(10), CALIFICACIÓN INTEGER(2))''')
except:
    pass
condition = True

os.system('cls')
# Se llaman las funciones de los archivos Graficador; FuncionesSQL y FuncipnesGraficas
